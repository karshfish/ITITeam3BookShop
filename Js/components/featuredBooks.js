...
export async function renderFeatureBooks() {
    ...
    try {
        ...
        books.forEach(book => {
            ...
            card.className = 'item d-flex flex-column align-items-center rounded text-center p-2';
            ...
            function goToBookPage(book) {
                ...
                location.href = `product-details.html?title=${encodeURIComponent(title)}`;
            }
            ...
            card.querySelector('.add-btn').addEventListener('click', function(event) {
