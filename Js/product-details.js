    ...
    detailRow.innerHTML = `
        <!-- Left Column: Image -->
                <div class="col-md-4 text-center">
                    <img src="${book.covers.large}" alt="${book.title}" class="book-image">
                </div>
                <!-- Right Column: Info -->
                <div class="col-md-8">
                    ...
                    <!-- Description -->
                    <p>${book.description?.value || book.description || "No description provided"}</p>
                    <!-- Author, Publisher, Year -->
                    <div class="d-flex flex-wrap mb-3">
                        <div>
                            <ul class="me-4">
                                <li>Author</li>
                                <li>${book.authors?.map(a => a.name).join(", ") || "Unknown"}</li>
                            </ul>
                        </div>
                        <ul class="me-4">
                            <li>Publisher</li>
                            <li>${book.publish.publishers?.map(p => p.name).join(", ") || "Unknown"}</li>
                        </ul>
                        <ul>
                            <li>Year</li>
...
function attachEventsHandlers(book) {
    ...
    cartIcon.addEventListener('click', () => {
        ...
        if (existingBook) {
            ...
            alert(`Quantity updated! Now you have ${existingBook.quantity} of this book in your cart.`)
        } else {
...
function showRecommendedBooks(currentBook) {
    ...
    let relatedHTML = "";
    randomSix.forEach(b => {
        const cover = b.covers?.large || b.covers?.medium || b.covers?.small || "placeholder.jpg";
        relatedHTML += `
