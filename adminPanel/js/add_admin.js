...
function addAdmin(adminData) {
  ...
  if (!validateName(adminData.fullName)) {
    return {
      success: false,
      message: "الاسم يجب أن يحتوي على 3 أحرف على الأقل وبدون رموز.",
    };
  }
  ...
  if (!validatePassword(adminData.password)) {
    return {
      success: false,
      message:
        "كلمة المرور ضعيفة! يجب أن تحتوي على حرف كبير وصغير ورقم ورمز خاص.",
    };
  }

  // التحقق من عدم تكرار اسم المستخدم أو البريد الإلكتروني
  const usernameExists = admins.some(
    (admin) => admin.fullName === adminData.fullName
  );
